...
def throwSimpPreprocessingError (e : Exception) : TacticM α :=
  throwError "hammer encountered error during simp preprocessing. Try calling hammer with the simpTarget option set to no_target. Error: {e.toMessageData}"
...
def hammerErrorIsSimpPreprocessingError (e : Exception) : IO Bool := do
  let eStr ← e.toMessageData.toString
  return "hammer encountered error during simp preprocessing. Try calling hammer with the simpTarget option set to no_target.".isPrefixOf eStr
